...
pub struct Platformer2dLabE2EPlugin;

impl Plugin for Platformer2dLabE2EPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(saddle_bevy_e2e::E2EPlugin);
        app.configure_sets(Update, E2ESet.before(saddle_character_state_machine::CharacterStateMachineSystems::GatherFacts));
        ...
        if let Some(name) = scenario_name {
            if let Some(mut scenario) = scenarios::scenario_by_name(&name) {
                ...
                error!(
                    "[platformer_2d:e2e] Unknown scenario '{name}'. Available: {:?}",
