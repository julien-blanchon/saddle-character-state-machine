...
use crate::bindings::{BevyAnimationBridge, CharacterAnimationSelection};
...
use crate::config::CharacterStateMachineLibrary;
...
pub(crate) fn apply_animation_selection(
    ...
    for (entity, machine_component, selection, bridge) in &machines {
        ...
        active_animation.set_repeat(binding.repeat.to_bevy());

        if let Some(duration_seconds) = binding.duration_seconds.filter(|duration| *duration > 0.0)
            && let Some(sync_normalized_time) = selection.sync_normalized_time
        {
